...
class Trainer:
    ...
    def _log_episode_info(self, info, global_step, epsilon):
        ...
        """
        print(f"global_step={global_step}, episode_counter={self.episode_counter}, episodic_return={info['r']}")
        if self.logger:
            self.logger.log_scalar("episode/reward", info["r"], self.episode_counter)
            self.logger.log_scalar("episode/length", info["l"], self.episode_counter)
            self.logger.log_scalar("epsilon", epsilon, self.episode_counter)
